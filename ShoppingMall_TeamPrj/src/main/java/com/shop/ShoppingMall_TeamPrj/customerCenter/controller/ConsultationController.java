...
public class ConsultationController {
 ...
 // 상담 목록 조회 (일반 사용자는 자신의 게시글, 관리자(ADMIN)는 전체 게시글)
    ...
    public String listConsultations(HttpServletRequest request, Model model) {
        ...
        if (session == null) {
            ...
            return "redirect:/member/loginForm.do?error=sessionNotFound";
        }

        // `memberInfo` 객체에서 `user_id`와 `role` 가져오기
        MemberVO memberInfo = (MemberVO) session.getAttribute("memberInfo");
        if (memberInfo == null) {
            System.out.println("ConsultationController: 세션에 memberInfo 없음. 로그인 필요.");
            return "redirect:/member/loginForm.do?error=loginRequired";
        }

        Integer userId = memberInfo.getUser_id();
 ...
 // 상담 문의 제출 처리
    ...
    public String submitConsultation(HttpServletRequest request, ConsultationVO consultationVO) {
        ...
        // `memberInfo`에서 `user_id`와 `role` 가져오기
        MemberVO memberInfo = (MemberVO) session.getAttribute("memberInfo");
        if (memberInfo == null) {
            System.out.println("ConsultationController: 세션에 memberInfo가 없습니다. 로그인 후 이용해 주세요.");
